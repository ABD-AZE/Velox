...
ExpressionNodePtr SemanticAnalyzer::convertTo(ExpressionNodePtr expr,
                                              Type &targetType)
{
  if (!expr || !expr->type)
    return expr;
...
void SemanticAnalyzer::visit(VarDeclNode &node)
{
  ...
  if (inFileScope)
  {
    ...
    auto InitNode =
        node.init.has_value()
            ...
            : nullptr;
    auto constInit = (InitNode && InitNode->kind == InitializerKind::SINGLE_INIT)
                         ? dynamic_cast<ConstantExpression *>(
                               std::get<SingleInit>(InitNode->data)
                                   .expression.get())
                         : nullptr;
  ...
  if (node.init)
  {
    ...
    if (auto *initExp =
            dynamic_cast<ExpressionNode *>(node.init.value().get()))
    {
      ...
      if (initExp->type && *initExp->type != node.type)
      {
        ...
        auto castExpr = std::make_unique<CastExpression>(
            node.type, std::move(node.init.value()));
