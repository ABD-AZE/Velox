...
ASTNodePtr Parser::parseDoWhile()
{
  ...
  // Check if it's 'while' or 'until'
  Token loopToken = consume();
  bool isUntil = false;
  if (loopToken.GetType() == TokenType::UNTIL)
  {
    isUntil = true;
  }
  else if (loopToken.GetType() == TokenType::WHILE)
  {
    isUntil = false;
  }
  else
  {
    expect(loopToken.GetType(), TokenType::WHILE); // This will report an error
  }
  ...
  // If it's 'until', negate the condition
  if (isUntil)
  {
    ASTNodePtr negatedCondition = std::make_unique<UnaryExpression>(
        TokenType::NOT, std::move(condition));
    return std::make_unique<DoWhileNode>(std::move(negatedCondition), std::move(body));
  }
...
ASTNodePtr Parser::parseUntil()
{
  expect(consume().GetType(), TokenType::UNTIL);
  expect(consume().GetType(), TokenType::OPEN_PARENTHESES);
  ASTNodePtr condition = parseExpression(0);
  expect(consume().GetType(), TokenType::CLOSE_PARENTHESES);
  ASTNodePtr body = parseStatement();
  // Create a negated condition: until(x) becomes while(!(x))
  ASTNodePtr negatedCondition = std::make_unique<UnaryExpression>(
      TokenType::NOT, std::move(condition));
  return std::make_unique<WhileNode>(std::move(negatedCondition), std::move(body));
}
