...
ASTNodePtr Parser::parseDoWhile() {
  ...
  if (isUntil) {
    ASTNodePtr negatedCondition =
        std::make_unique<UnaryExpression>(TokenType::NOT, std::move(condition));
    return std::make_unique<DoWhileNode>(std::move(negatedCondition),
                                         std::move(body));
  }
...
ASTNodePtr Parser::parseUntil() {
  ...
  ASTNodePtr negatedCondition =
      std::make_unique<UnaryExpression>(TokenType::NOT, std::move(condition));
  return std::make_unique<WhileNode>(std::move(negatedCondition),
                                     std::move(body));
}
