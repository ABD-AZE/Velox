 ...
 */
long negative_one =
    1l; // can't use negative static initializers; negate this in main
long zero = 0l;
...
int main(void) {
  ...
  /* constants with ul suffix are always treated as unsigned long, not unsigned
   * int If these constants were interpreted as unsigned ints, addition would
   * wrap around to 0
   */
  if (!(3ul + 4294967293ul)) {
    return 3;
