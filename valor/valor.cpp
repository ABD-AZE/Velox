...
void IRGenerator::convertSymbolTableToIR() {
  ...
  for (const auto &[name, entry] : global_symbol_table) {
    ...
    if (entry.type.kind == TypeKind::ARRAY) {
      // Check if this is a string constant (symbolType is CONSTANT for string
      // literals)
...
void IRGenerator::processCompoundInitializer(InitializerNode *init,
  ...
  if (init->kind == InitializerKind::SINGLE_INIT) {
    ...
    if (varType.kind == TypeKind::ARRAY) {
      ...
      auto it = type_table.find(structTag);
      if (it == type_table.end()) {
        std::cerr << "Error: Struct " << structTag << " not found in type table"
                  << std::endl;
        return;
      }
...
void IRGenerator::visit(VarDeclNode &node) {
  ...
  if (node.init) {
    ...
    if (initNode && node.type.kind == TypeKind::ARRAY) {
      ...
      if (initNode->kind == InitializerKind::COMPOUND_INIT) {
        // Process compound initializer with CopyToOffset instructions for each
        // member
        processCompoundInitializer(initNode, node.name, node.type, 0);
      } else {
        // Single initializer for struct - shouldn't happen normally but handle
        // it
        (*node.init)->accept(*this);
        IRValuePtr initValue =
            currentExpResult.type == ExpResultType::PLAIN_OPERAND
...
void IRGenerator::visit(BinaryExpression &node) {
  ...
  if ((node.op == TokenType::PLUS || node.op == TokenType::HYPHEN)) {
    ...
    if (node.op == TokenType::PLUS && leftExpr && leftExpr->type &&
      ...
      currentExpResult = ExpResult::makePlainOperand(currentValue);
      return;
    } else if (node.op == TokenType::PLUS && rightExpr && rightExpr->type &&
               rightExpr->type->kind == TypeKind::POINTER && leftExpr &&
               leftExpr->type && leftExpr->type->kind != TypeKind::POINTER) {
      // integer + pointer
      ...
      auto negInst = IRInstructionNode::makeUnary(IROpType::NEGATE,
                                                  negatedRight, rightValue);
...
void IRGenerator::visit(AssignmentExpression &node) {
  ...
  if (leftResult.type == ExpResultType::SUBOBJECT) {
    ...
    auto copyInst = IRInstructionNode::makeCopyToOffset(
        rightValue, leftResult.base, leftResult.offset);
    ...
    while (leftResult.inner) {
      currentFunction->addInstruction(
          IRInstructionNode::makeLoad(leftValue, leftValue));
...
IRValuePtr IRGenerator::convertExpResult(ExpResult &result,
                                         const Type &exprType) {
  if (result.type == ExpResultType::PLAIN_OPERAND) {
    ...
    auto copyInst =
        IRInstructionNode::makeCopyFromOffset(result.base, result.offset, dst);
    currentFunction->addInstruction(copyInst);
    return dst;
  } else {
    IRValuePtr dst = makeTackyVariable(exprType);
    IRValuePtr src = result.value;
    IRValuePtr ptrdst = makeTackyVariable(Type::ULong());
    while (result.inner) {
      currentFunction->addInstruction(IRInstructionNode::makeLoad(src, ptrdst));
      src = ptrdst;
      result = *result.inner;
    }
    // Dereferenced pointer - emit Load instruction
    auto loadInst = IRInstructionNode::makeLoad(src, dst);
...
void IRGenerator::visit(CastExpression &node) {
  ...
  if (currentExpResult.type == ExpResultType::PLAIN_OPERAND)
    result = std::make_shared<IRValueNode>(*currentValue);
  else {
    // Need to load the value from the dereferenced pointer
    result = convertExpResult(currentExpResult, *exp->type);
  }
...
void IRGenerator::visit(DereferenceExpression &node) {
  ...
  if (node.pointerExpr) {
    ...
    auto ptr = currentExpResult;

    // Store as dereferenced pointer in currentExpResult
    // The actual Load will happen during lvalue conversion if needed
    if (ptr.type != ExpResultType::DEREFERENCED_POINTER)
      currentExpResult = ExpResult::makeDereferencedPointer(ptr.value, nullptr);
    else {
      currentExpResult = ExpResult::makeDereferencedPointer(
          ptr.value, std::make_shared<ExpResult>(ptr));
    }
    // // For compatibility, also store in currentValue (will be loaded if
    // needed) currentValue = ptr;
  }
}
    ...
    if (derefExpr) {
      ...
      if (currentExpResult.type == ExpResultType::SUBOBJECT) {
        ...
        if (currentExpResult.offset != 0) {
          IRValuePtr offsetValue = IRValueNode::makeConstant(
              static_cast<long>(currentExpResult.offset));
          IRValuePtr resultPtr = makeTackyVariable(Type::ULong());
          currentFunction->addInstruction(IRInstructionNode::makeAddPtr(
              basePtr, offsetValue, 1, resultPtr));
        ...
        while (currentExpResult.inner) {
          currentFunction->addInstruction(
              IRInstructionNode::makeLoad(src, src));
...
void IRGenerator::visit(ForInit &node) {
  (void)node; /* TODO: Implement for loop initializations */
}
...
void IRGenerator::visit(SubscriptExpression &node) {
  ...
  // index)
  currentExpResult = ExpResult::makeDereferencedPointer(result, nullptr);
  currentValue = result;
}
...
void IRGenerator::visit(DotExpression &node) {
  ...
  if (!structExpr || !structExpr->type ||
      structExpr->type->kind != TypeKind::STRUCT) {
  ...
  if (innerObject.type == ExpResultType::PLAIN_OPERAND) {
    ...
    currentExpResult = ExpResult::makeSubObject(
        innerObject.base, innerObject.offset + memberOffset);
    ...
    if (memberOffset != 0) {
      IRValuePtr offsetValue =
          IRValueNode::makeConstant(static_cast<long>(memberOffset));
      IRValuePtr resultPtr = makeTackyVariable(Type::ULong());
      currentFunction->addInstruction(
          IRInstructionNode::makeAddPtr(ptr, offsetValue, 1, resultPtr));
      ptr = resultPtr;
    }
...
void IRGenerator::visit(ArrowExpression &node) {
  ...
  if (!pointerExpr || !pointerExpr->type ||
      pointerExpr->type->kind != TypeKind::POINTER) {
  ...
  if (memberOffset != 0) {
    ...
    IRValuePtr resultPtr = makeTackyVariable(Type::ULong());
    currentFunction->addInstruction(
        IRInstructionNode::makeAddPtr(ptrValue, offsetValue, 1, resultPtr));
    ptrValue = resultPtr;
  }
